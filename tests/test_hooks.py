...
# ---------------------------------------------------------------------------
# Async versions of the hook‑interaction tests
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_post_fetch_mutation_async(monkeypatch):
    """post_fetch can rewrite the HTML before parse (async core)."""
    class RewriteTitle:
    ...
    # breakpoint()
    val = [v async for v in async_core.evaluate_wxpath_bfs_iter_async(None, segs, max_depth=0)][0]
    assert str(val) == "Rewritten"


@pytest.mark.asyncio
async def test_post_fetch_prunes_branch_async(monkeypatch):
    """post_fetch returning None stops branch traversal (async core)."""
    class Prune:
        async def post_fetch(self, ctx, html_bytes):
            if b"The B Page" in html_bytes:
                return None
    ...
    segs = parser.parse_wxpath_expr(expr)
    results = [
        e async for e in async_core.evaluate_wxpath_bfs_iter_async(None, segs, max_depth=1)
    ]
    assert [e.base_url for e in results] == ["http://c/", "http://d/"]


@pytest.mark.asyncio
async def test_post_parse_prunes_branch_async(monkeypatch):
    """post_parse returning None stops branch traversal (async core)."""
    class Prune:
    ...
    monkeypatch.setattr(
        helpers,
        "fetch_html",
        _generate_fake_fetch_html({"http://a/": HTML_A}),
    )
    expr = "url('http://a/')///url(@href)"
    segs = parser.parse_wxpath_expr(expr)
    results = [
