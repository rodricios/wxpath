...
def parse_wxpath_expr(path_expr):
    ...
    for s in segments:
        ...
        if s.startswith('url("') or s.startswith("url('"):
            parsed.append(('url', _extract_arg_from_url_xpath_op(s)))
    ...
    for i in range(len(parsed) - 1):
        if parsed[i][0] == 'inf_xpath' and parsed[i + 1][0] == 'url_from_attr':
            ...
            url_from_attr_value = _extract_arg_from_url_xpath_op(parsed[i + 1][1])
...
def evaluate_wxpath_bfs_iter(elem, segments, max_depth=1, seen_urls=None, curr_depth=0, html_handlers=[]):
    ...
    """
    assert max_depth >= (_count_ops_with_url(segments) - 1) , "max_depth+1 must be equal to or greater than the number of url* segments. " +\
